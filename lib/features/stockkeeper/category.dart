...
Future<void> _loadUserId() async {
  try {
    // Use the custom method to get the user ID
    final storedUserId = await _secureStorage.getUserId();  // Await the result
    setState(() {
      userId = storedUserId; // Set userId if found
      print(' category: Loaded userId: $userId');
    });
  } catch (e) {
...
Future<void> _logout() async {
  try {
    ...
    await _secureStorage.clear();  // Corrected: no need to pass the key
  ...
  Future<void> _addCategory() async {
    ...
    try {
      ...
      final category = Category(
        ...
        // Add userId to the category if needed
        // userId: userId,
      );
  ...
  // Validate category name - CHANGED TO SYNCHRONOUS
      ...
      appBar: AppBar(
        ...
        actions: [
          if (userId != null)
            IconButton(
              onPressed: _logout,
              icon: const Icon(Feather.log_out, color: kText),
              tooltip: 'Logout',
            ),
        ],
      ),
            ...
            SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 60), // Extra space for user ID at bottom
                ],
              ),
            ),

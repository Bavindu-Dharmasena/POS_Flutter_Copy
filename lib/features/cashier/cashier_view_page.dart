...
class _CashierViewPageState extends State<CashierViewPage> {
  ...
  void _printBill({
    ...
    // 2) Build invoices payload (needs itemId, batchId, quantity)
    // final List<Map<String, dynamic>> invoicesPayload = cartItems
    //     .where((i) => i['itemId'] != null && i['batchID'] != null)
    //     .map<Map<String, dynamic>>(
    //       (i) => {
    ...
    //       final dynamic rawItemId = i['itemId'];
    ...
    //       String batchId = (i['batchID'] ?? i['batchId'] ?? 'quick')
    //           .toString()
    //           .trim();
    //       if (batchId.isEmpty) batchId = 'quick';

    //       // quantity: int (default 0 if missing/unparsable)
    //       final int qty = (i['quantity'] is num)
    //           ? (i['quantity'] as num).toInt()
    //           : int.tryParse('${i['quantity']}') ?? 0;

    //       return {'batchId': batchId, 'itemId': itemId, 'quantity': qty};
    //     })
    //     .toList();
    ...
    //   final lineTotal = finalUnitPrice * qty;
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to create invoices: $e')));
      return;
    }
